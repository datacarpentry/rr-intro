...
ggplot(gap_north_america, aes(x = year, y = lifeExp, color=country)) + geom_line() + xlab("Year") + ylab("Life Expectancy")
```
...
ggplot(gap_europe, aes(x=gdpPercap, y = lifeExp)) + geom_point() + xlab("GDP (per capita)") + ylab("Life Expectancy")
```

```{r}
gap_2007 <- subset(gap_combined, year == 2007)

ggplot(gap_2007, aes(x=gdpPercap, y = lifeExp, group=continent, color=continent)) + geom_point() + xlab("GDP (per capita)") + ylab("Life Expectancy")
```
