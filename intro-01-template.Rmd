...
The first time you need a package you need to first download and install it. The
function `require()` tests whether a package can be loaded (and gets loaded if
it is available locally). The code below checks if the package is installed, if
not, it gets installed (and loaded).

```{r, eval=FALSE}
## this needs to be run interactively, otherwise the package
## doesn't get installed in the appropriate library.
if (!require("ggplot2")) {
